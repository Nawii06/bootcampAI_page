...
interface Target { id: string; indicatorId: string; targetValue: string; version: string }
interface Result { id: string; indicatorId: string; actualValue: string; status: string }
interface Overview { indicators: Indicator[]; targets: Target[]; results: Result[] }
...
export default function AdminPerformanceDashboard() {
  ...
  const overview = useQuery({
    ...
    queryFn: () => customFetch<Overview>(`/api/v1/performance/overview?businessYearId=${yearId}`, {
