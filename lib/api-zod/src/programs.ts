...
const ProgramSessionFieldsSchema = z.object({
...
export const ProgramSessionInputSchema = ProgramSessionFieldsSchema
...
export const ProgramUpdateSchema = ProgramInputSchema
  .omit({ businessYearId: true, code: true })
  .partial()
  .refine((value) => Object.keys(value).length > 0, {
    message: "변경할 프로그램 값이 필요합니다.",
  });

export const ProgramSessionUpdateSchema = ProgramSessionFieldsSchema
  .partial()
  .refine((value) => Object.keys(value).length > 0, {
    message: "변경할 회차 값이 필요합니다.",
  });
...
const ProgramSessionResponseSchema = z.object({
  id: z.string().uuid(),
  sequence: z.number().int().positive(),
  name: z.string(),
  capacity: z.number().int().positive(),
  applicationStartsAt: z.string(),
  applicationEndsAt: z.string(),
  startsAt: z.string(),
  endsAt: z.string(),
...
export const ProgramResponseSchema = z.object({
  ...
  eligibilityRules: ProgramInputSchema.shape.eligibilityRules,
  completionRules: ProgramInputSchema.shape.completionRules,
  status: z.string(),
  programSessions: z.array(ProgramSessionResponseSchema).optional().default([]),
}).passthrough();
...
export const ProgramApplicationResponseSchema = z.object({
  id: z.string().uuid(),
  sessionId: z.string().uuid(),
  studentId: z.string().uuid().optional(),
...
export const ProgramApplicationsResponseSchema = z.object({
  data: z.array(ProgramApplicationResponseSchema),
});
...
export type ProgramApplicationResponse = z.infer<
  typeof ProgramApplicationResponseSchema
>;
